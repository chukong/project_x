# Cocos Studio v2.0 Product Requirements Document
v0.3

## Revisions

v0.3 - Replaced _Cocos_ with _Cocos Studio v2.0_. Adds the different between Cocos Studio v1.0 and v2.0
...
Although Cocos2d-x vision is still valid for Cocos Studio v2.0, it is worth adding the "easy-to-use" in it, since the User Experience is a very important part of Cocos Studio v2.0. The updated vision is:

    Cocos Studio is a complete, easy to use, toolchain for developing multi-platform games
    which goes all the way from rapid prototyping to a finished high performing game
 ...
 * It is easy to integrate Cocos Studio with 3rd party libraries
...
* Easy to monetize: Cocos Studio should provide a way to monetize games by providing integration with Payment, Ads, Cross Promotion and similar services
...
To describe Cocos Studio using different words, Cocos Studio is everything that is between the "I have this idea for a new game" and the published game in different Stores.

![](./graphic.png)

But Cocos Studio is NOT just a collection of editors and services. Cocos Studio might include different editors and services, but it is __key__ that these editors and services are __well integrated__: smooth workflow; and familiar, easy-to-use User Experience.
...
So, what is Project Anarchy ? It is a collection of great editors and tools, but without any kind of integration. The result is that Project Anarchy is very difficult to use. Definitely we don't want this for Cocos Studio.



### Cocos Studio components

The following are the high level components of Cocos Studio v2.0:
...
Also, a game developer only needs one and only one Developer Account in order to use Cocos Studio.
...
__One and only one Developer Account__ for all the Cocos Studio components.

Additionally, the Cocos Studio product also needs __marketing material__, like logo, name, brochures, white papers, etc.

### Components in Detail

#### Game Editor

The Game Editor is the __most important__ piece of Cocos Studio because it is the 1st thing our users will use, and also because users will spend almost all their time using this tool to create their games. So it is very important that the Game Editor is very polished.

For the Game Editor, Cocos Studio v2.0 will initially use:

* Cocos Studio v1.x (our current game editor)
* Cocos Code IDE
* Cocos Command Line Tool
* Launcher

_Cocos Studio v1.x_ and _Cocos Code IDE_ are two different editors, that are currently not integrated, that were developed using two different technologies, so their look and feel is completely different with completely different UI and UX. This is something that we should try to avoid.
...
Feature-wise, Cocos Studio v1.x and Cocos Code IDE are good, although there are still a lot of things that need to be added. But the current set of features is good. What these editors need is to be polished, in particular Cocos Studio v1.x. A lot of effort needs to be put in polishing Cocos Studio v1's workflow and its UX.
...
* It is used as a common code base: Cocos Studio v1.x and Cocos Code IDE might want to create a new game. We don't need to duplicate the code. They will just use the Cocos Command Line Tool
* It is faster to develop since it is in python

___Creating New Games___:

When a new game is created, a Cocos Project File must be created. Double-Clicking on that file, must open Cocos Editor (Cocos Studio v1.x in this case).
...
Additionally, Cocos Studio v2.0 must have _Templates_ that the user can choose from when creating a new game. As an example, these are possible templates:
...
For the Game Engine, Cocos Studio will use Cocos2d-x and Cocos2d-JS.

Since Cocos Studio is also a platform, and our users will depend on it, it is important that Cocos Studio is backwards compatible. It is important to have a good, easy to use API, but it is more important to have an stable API. 
...
* If no, wouldn't it be confusing for the Cocos Studio users ?
...
___Logo___:
We could keep using the current Cocos Studio logo.

![Cocos Studio logo](./cocos_studio_logo.png)

___Name___:

_Cocos Studio_ is a very good name.

The only issue, apparently, is that Cocos Studio v1.x is not well regarded in China is it has many issues.

___White Papers___:
Since Cocos Studio uses Cocos2d-X as its game engine, and Cocos2d-X is very fast, much faster than Unity and SpriteKit, we need to publish that information on the in the site.
...
* blog: with daily/weekly news about Cocos Studio and Game development in general
* about: What is Cocos Studio, and why you need it
* download information
* forum: For discussion
* help: programmers guide and API reference

The site http://cocos2d-x.org should not be abandoned. We should keep maintaining it and posting news related to cocos2d-x. But the cocos2d-x site is different than the Cocos Studio site.


#### Developers Account

As mentioned earlier we need one and only one Developers Account to use Cocos Studio.
...
What we need is one unified Developers Account. Once our users create an account, it must be valid to use any component of Cocos Studio, and no other acount must be needed.
...
Cocos Studio must be easy to install. 

#### Auto-update

Cocos Studio must be able to auto-update itself. Daily or weekly, Cocos Studio must check if there is a new version available. If so, it should tell the user and ask him if he wants to install the new version.
...
* Users from other engines that finds Cocos Studio a better product
* New game developers
...
The first public version of Cocos Studio (v2.0-beta0) must be ready before the Cocos Developers Conference, to be held somewhere in September.

Cocos Studio v2.0-beta0 must include:
...
* Developers Account v0.1. Everything must use one account. In order to use Cocos Studio, users must register.
...
__What's not part of v2.0-beta0__:

For Cocos Studio v2.0-beta0, Cocos2d-JS won't be supported. Cocos2d-JS will be added in the future, perhaps in v2.1 or v2.2.
...
Cocos2d-JS development MUST continue. Lot's of users are using it, and is very important for the company to keep developing it. The only problem right now is that integrating it with Cocos Studio will take a lot of time. Cocos2d-JS is much more complex than Lua because it also supports Web.
